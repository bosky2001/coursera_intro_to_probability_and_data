...
Looking at histogram 2 we can see a lot the delays are 0 to 15 and a few belong to -15 to 0. It is harder to tell in the first and third histogram that there are negative values. since they are binned with the positives.
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(med = median(arr_delay),
            iqr = IQR(arr_delay)) %>%
  top_n(1, iqr)
```
...
based on whether the flight departed on time or not. What proportion of flights that were `"delayed"` departing arrive `"on time"`? 
...
nycflights %>%
  ...
  summarise(arr = mean(arr_type == 'on time'))
```
